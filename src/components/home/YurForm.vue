  ...
  <form action="jismoniy">
    ...
    <div class="pb-6">
      ...
      <input
        type="text"
        id="first_name"
        v-model="yuridikForm.tashkilot"
        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
        placeholder="Magic Soft MCHJ"
        required
      />
    </div>
    <button @click.prevent="yurFormFunc" class="w-full py-2 bg-blue-500 text-white rounded-md text-center">
      Yuborish
    </button>
  </form>
</template>
...
import { PaySum, YuridikForm } from "../../types/PersonForm";
...
const yuridikForm = ref<YuridikForm>({
  fish: "",
  tel: "",
  pay: 1000000,
  tashkilot: "",
...
const handlePay = (id: number) => {
  yuridikForm.value.pay = paySum[id].num;
